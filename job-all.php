			...
			<table class="table table-striped my-4 w-100" id="datatable1">
				...
				<tbody>
					...
					$sql	=	"
								SELECT		*
								FROM		iservices
								WHERE		service_Date_Start	>	'2019-01-01'
					...
					while($row	=	mysqli_fetch_array($query,MYSQLI_ASSOC))
					{
						?>
						<tr>
							<td> <a target="_blank" href="job-detail.php?service_ID=<?php echo $row['service_ID']; ?>"><?php echo $row['service_ID']; ?></a> </td>
							<td><?php echo $row['service_Date_Start']; ?></td>
							<td><?php echo get_sec($row['sec_ID']); ?></td>
							<td>
								<?php 
								if($row['dep_ID']	==	999)
								{
							...
							<td><?php echo get_status($row['status_ID']); ?></td>
						</tr>
						<?
					}
					?>
