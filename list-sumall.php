	...
	<form action="" method="GET">
		<div class="row">
			...
			<div class="col-2">
				...
				<a href="list-report.php" type="button" class="btn btn-dark">Clear</a>
			</div>
		</div>
	</form>
	<hr>
			...
			<table class="table table-bordered table-striped my-4 w-100" id="datatablehw">
					...
					$query	=	mysqli_query($connect,$sql) or die(mysqli_error());
						while($row	=	mysqli_fetch_array($query,MYSQLI_ASSOC))
						{
							...
							<tr>
								...
								{
									?>
									<td><?php echo get_asset_count($row['com_ID'],$row2['asset_type_ID'],$row2['asset_subtype_ID']); ?></td>
									<?
								}
								?>
								<td><?php echo get_asset_countall($row['com_ID'],'all'); ?></td>
							</tr>
							<?
						}
					?>
				...
				<tfoot>
					<tr>
						...
						{
							?>
							<th><?php echo get_asset_counttotal($row['asset_type_ID'],$row['asset_subtype_ID']); ?></th>
							<?
						}
						?>
						<th width="10%"><?php echo get_asset_countalltotal('all'); ?></th>
					</tr>
				</tfoot>
			</table>
		</div>
