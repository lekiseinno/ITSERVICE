		...
		<div class="card-body">
			<table class="table table-bordered table-striped my-4 w-100" id="datatable1">
				...
				<tbody>
					...
					$sql	=	"
								SELECT		*
								FROM		wh_item_transection
								ORDER BY	item_use_Date	ASC
								";
					...
					while($row	=	mysqli_fetch_array($query,MYSQLI_ASSOC))
					{
						?>
						<tr>
							<td><?php echo $row['item_use_ID']; ?></td>
							<td><?php echo $row['item_use_Date']; ?></td>
							<td><?php echo get_item($row['item_ID']); ?></td>
							<td><?php echo getstatus($row['event_transection']); ?></td>
							<td><?php echo $row['item_use_QTY']; ?></td>
							<td><?php echo $row['item_use_name']; ?></td>
						</tr>
						<?
					}
					?>
...
function getstatus($t)
{
	if($t=='Import')
	{
		$data	=	'<span class="badge badge-success">Import</span>';
	}
	if($t=='Export')
	{
		$data	=	'<span class="badge badge-danger">Export</span>';
	}
	return $data;
}
?>
